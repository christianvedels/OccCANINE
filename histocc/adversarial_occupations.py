...
def eta(i, start_time, capN):
    ...
    total_seconds = capN * average_time_per_n
    ...
    eta_str = f"{i} of {capN} {eta_str} of {total_str}"
...
class AdversarialStrings():
    ...
    """
    def __init__(self, attacker, translator, predictor, df, verbose = True):
        """
    ...
    def _run_OccCANINE_predicts(self, text, lang):
    ...
    def get_adv_string(self, text, lang, labels, i, verbose_extra = False, n_max = 100, alt_prob = 1, n_trans=3, double_translate = True):
        ...
        self.lang = lang
        if lang == 'en':
            NotImplementedError("English not implemented yet. Would translate from English to English")
...
def generate_adversarial_wrapper(toyload=False, double_translate = True, verbose_extra=False, sample_size = 1000, n_max = 10):
    ...
    for idx, row in df.iterrows():
        ...
        print(f"{eta(i=i, start_time=start_time, capN=capN)} --> Stopped after {n_augs} of {n_max}")
        i += 1
