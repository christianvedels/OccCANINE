...
def balance_classes(df, system = 'hisco'):
    ...
    """
    raise NotImplementedError("This funciton is just autogenerated crap for now.")

    # Get the counts of each class
    class_counts = df[f'{system}_1'].value_counts()

    # Find the minimum count among all classes
    min_count = class_counts.min()

    # Sample from each class to create a balanced DataFrame
    balanced_df = pd.concat([df[df[f'{system}_1'] == cls].sample(min_count) for cls in class_counts.index])
