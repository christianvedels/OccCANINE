...
files = list.files("Project_dissemination/Paper_replication_package/Data/Intermediate_data/test_performance/source", full.names = TRUE) %>% sample(replace = FALSE)
start_time = Sys.time()
foreach(i = seq_along(files), .combine = "bind_rows") %do% {
    ...
    eval_data = read_csv(f, show_col_types = FALSE, guess_max = 1000000, progress = FALSE) %>%
        mutate(file = f) %>%
        mutate(file = gsub("Project_dissemination/Paper_replication_package/Data/Intermediate_data/test_performance/source/", "", file)) %>%
        ungroup()
