...
def evaluate(
    ...
    for batch_idx, batch in enumerate(data_loader, start=1):
        ...
        targets = batch['targets_seq2seq'].to(device)
    ...
    # Map linear predictions into system-codes
    map_code_system_code = {v: k for k, v in data_loader.dataset.map_code_label.items()}

    preds_linear = []

    for i in range(linear_topk):
        preds_linear.append(list(map(
            map_code_system_code.get,
            preds_linear_raw[:, i],
        )))
