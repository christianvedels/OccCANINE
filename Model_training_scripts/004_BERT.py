...
The function randomly changes letters according to an 'ALT_PROB'.
The function also randomly inserts a word in a random location in each string, 
with the same 'ALT_PROB'. The words are drawn from the distribution
of words in the training data. Attack() is only activated once the 
guess-frequency local minimum is passed by 10%. 


BERT hyperparameter reccomendations: 
...
for b in range(3, 8):
    for k in range(3, 7):
        ...
        train_data_loader_attack = create_data_loader(df_train, tokenizer, MAX_LEN, BATCH_SIZE, N_CLASSES, alt_prob = ALT_PROB, insert_words = True)
        ...
        def plot_progress(train_losses, val_losses, step):
            ...
            plt.savefig(f"Tmp traning plots/loss_BERT_{model_domain}_sample_size_{sample_size}_lr_{LEARNING_RATE}_batch_size_{BATCH_SIZE}.png")
            plt.show()
            plt.close()
        ...
        for epoch in range(EPOCHS):
            ...
            if val_acc > best_accuracy:
                torch.save(
                    model.state_dict(), 
                    f'Trained_models/BERT_{model_domain}_sample_size_{sample_size}_lr_{LEARNING_RATE}_batch_size_{BATCH_SIZE}.bin'
                    )
